...
type Supervisor interface {
	CreateGameWithPlayers(gameID string, players []string) error
}
...
func (lm *LobbyManager) TransitionLobbyToGame(lobbyID string) error {
	...
	// Get a list of player IDs to pass to the game actor
	playerIDs := make([]string, len(lobby.Players))
	for i, p := range lobby.Players {
		playerIDs[i] = p.ID
	}

	// Tell the supervisor to create the GameActor
	err := lm.supervisor.CreateGameWithPlayers(lobbyID, playerIDs)
	if err != nil {
