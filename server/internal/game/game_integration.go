...
func (gm *GameManager) ProcessDayPhase() error {
	...
	if gm.shouldTriggerCrisis() {
		crisis, additionalEvents := gm.CrisisManager.TriggerRandomCrisis()
		if crisis != nil {
			...
			*gm.GameState = newState
			...
			// Apply any additional events generated by the crisis (e.g., role revelations)
			for _, event := range additionalEvents {
				newState = core.ApplyEvent(*gm.GameState, event)
				*gm.GameState = newState
				log.Printf("Applied crisis effect event: %s", event.Type)
			}
		}
	}
