...
We will **not** implement the full CQRS and event-sourcing architecture for the V1 release. We will stick to the existing, simpler actor-and-manager model. The focus for V1 will be on making the _current_ architecture as robust as possible within its own paradigm. This includes: 1. Merging the `LobbyManager` and `SessionManager` into a unified `GameLifecycleManager` to clarify ownership and simplify handoffs. 2. Implementing robust garbage collection for stale lobbies and abandoned games. 3. Hardening the existing components with stricter validation, rate limiting, and resource caps.
