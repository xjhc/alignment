...
*   **Optimization:** This structure is "cache-friendly." By placing the large, unchanging static instructions first, we enable Language Model providers to cac# Feature: AI Prompt Strategy & Persona Design
...
*   **Source of Truth:** The entire system is implemented using an "In-Code Prompt Registry" where all prompts are defined as Go structs, as detailed in the **[Prompt Management & Delivery](../architecture/08-prompt-management-and-delivery.md)** architecture document.he its tokenized representation, significantly reducing processing latency on subsequent API calls.
