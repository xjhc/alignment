    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.socket.onopen = () => {
          ...
          // If we have reconnection data, send RECONNECT action
          if (gameId && playerId && sessionToken && lastEventId) {
            this.sendAction({
              type: 'RECONNECT',
              payload: {
                game_id: gameId,
  ...
  private applyEventToGameEngine(serverEvent: ServerEvent): void {
    ...
    try {
      ...
      const coreEvent: CoreEvent = {
        ...
        gameId: serverEvent.payload?.game_id || 'unknown',
        playerId: serverEvent.payload?.player_id || '',
        timestamp: new Date().toISOString(),
        payload: serverEvent.payload || {},
      };

      // Apply to game engine if it's ready
      if (gameEngine.isReady()) {
        gameEngine.applyEvent(coreEvent).catch(error => {
