...
export const NightActionSelection: React.FC<NightActionSelectionProps> = () => {
  ...
  const [shadowTarget, setShadowTarget] = useState<string>(''); // Final target for copied ability
  const [shadowStep, setShadowStep] = useState<'select-source' | 'select-target'>('select-source');
  ...
  const eligibleShadowTargets = alivePlayers.filter(p => 
    p.role?.type && p.projectMilestones >= 3 && 
    ['CISO', 'CTO', 'CEO', 'COO', 'CFO', 'ETHICS', 'PLATFORMS'].includes(p.role.type)
  );
  ...
  const handleActionSelect = (action: ActionType) => {
    ...
    if (action === 'project') {
      ...
      // BOOTCAMP doesn't need a target
      handleInternBootcamp();
      setIsMinimized(true);
    }
  };
  ...
  const handleTargetSelect = (targetId: string) => {
    if (selectedAction === 'shadow') {
      if (shadowStep === 'select-source') {
        ...
        handleInternShadow(selectedTarget, targetId);
        setIsMinimized(true);
      }
    } else {
      setSelectedTarget(targetId);
  ...
  // Intern action handlers
  const handleInternBootcamp = () => {
    // Submit BOOTCAMP action
    const actionData = {
      type: 'BOOTCAMP',
    };
    // This would be sent to the backend - using existing game actions pattern
    console.log('BOOTCAMP action submitted:', actionData);
    // TODO: Integrate with actual game actions API
  };

  const handleInternShadow = (sourcePlayerId: string, finalTargetId: string) => {
    // Submit SHADOW action with two-step targeting
    const actionData = {
      type: 'SHADOW',
      targetId: sourcePlayerId, // Player to shadow
      payload: {
        shadow_target_id: finalTargetId, // Final target for the copied ability
      },
    };
    console.log('SHADOW action submitted:', actionData);
    // TODO: Integrate with actual game actions API
  };
        ...
        <div className="p-2">
          <div className="flex flex-col gap-1">
            ...
            {isIntern ? (
              <>
                <div className={`flex items-center gap-1.5 px-2 py-1.5 bg-gray-700 border border-gray-600 rounded-md cursor-pointer transition-all duration-150 hover:bg-gray-600 ${
                  selectedAction === 'bootcamp' ? 'bg-amber-500/10 border-amber-500' : ''
                }`} onClick={() => handleActionSelect('bootcamp')}>
                  <span className="text-xs w-4 text-center">📚</span>
                  <span className="font-medium text-xs text-gray-100 flex-grow">Bootcamp</span>
                  <span className="text-xs text-blue-500 bg-blue-500/10 px-1 py-0.5 rounded-lg font-medium">+1 Point</span>
                </div>
                <div className={`flex items-center gap-1.5 px-2 py-1.5 bg-gray-700 border border-gray-600 rounded-md transition-all duration-150 ${
                  canUseShadow ? `cursor-pointer hover:bg-gray-600 ${selectedAction === 'shadow' ? 'bg-amber-500/10 border-amber-500' : ''}` : 'opacity-60 cursor-not-allowed grayscale-30'
                }`} onClick={() => canUseShadow && handleActionSelect('shadow')}>
                  <span className="text-xs w-4 text-center">👤</span>
                  <span className="font-medium text-xs text-gray-100 flex-grow">Shadow</span>
                  <span className="text-xs text-green-500 font-bold">{canUseShadow ? 'Ready' : `Need ${1 - bootcampPoints} pts`}</span>
                </div>
              </>
            ) : (
              <div className={`flex items-center gap-1.5 px-2 py-1.5 bg-gray-700 border border-gray-600 rounded-md transition-all duration-150 ${
        ...
        <div className="flex flex-col gap-2">
          ...
          {isIntern ? (
            <>
              <div 
                className={`px-3 py-2 rounded-lg border border-gray-600 bg-gray-800 cursor-pointer transition-all duration-150 hover:bg-gray-700 hover:-translate-y-0.5 ${
                  selectedAction === 'bootcamp' ? 'border-amber-500 bg-amber-500/10' : ''
                }`}
                onClick={() => handleActionSelect('bootcamp')}
              >
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-base w-5 text-center">📚</span>
                  <span className="font-semibold text-gray-100 text-sm flex-grow">Bootcamp Training</span>
                  <span className="text-xs text-blue-500 bg-blue-500/10 px-1.5 py-0.5 rounded-2xl font-bold">+1 Point</span>
                </div>
                <div className="text-xs text-gray-400 leading-snug mb-1.5">
                  Spend the night in intensive training. Gain 1 Bootcamp Point that can be spent to copy another player's role ability.
                  Current Points: {bootcampPoints}
                </div>
                <div className="text-xs text-gray-500">
                  <strong className="text-gray-100 font-semibold">Command:</strong> <code className="bg-gray-700 px-1 py-0.5 rounded font-mono text-xs">Bootcamp Training</code>
                </div>
              </div>
              ...
              <div 
                className={`px-3 py-2 rounded-lg border border-gray-600 bg-gray-800 transition-all duration-150 ${
                  canUseShadow 
                    ? `cursor-pointer hover:bg-gray-700 hover:-translate-y-0.5 ${selectedAction === 'shadow' ? 'border-amber-500 bg-amber-500/10' : ''}` 
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-base w-5 text-center">👤</span>
                  <span className="font-semibold text-gray-100 text-sm flex-grow">Shadow Ability</span>
                ...
                <div className="text-xs text-gray-500">
                  <strong className="text-gray-100 font-semibold">Command:</strong> <code className="bg-gray-700 px-1 py-0.5 rounded font-mono text-xs">Shadow [Player] targeting [Target]</code>
                </div>
              </div>
            </>
          ) : (
        ...
        {selectedAction === 'shadow' && canUseShadow && (
          <div className="pt-3 border-t border-gray-600">
            {shadowStep === 'select-source' && (
              <>
                <div className="mb-2">
                  <h4 className="text-xs font-semibold text-gray-100 m-0">Step 1: Select Player to Shadow</h4>
                  <p className="text-xs text-gray-400 mt-1">Choose a player whose ability you want to copy. They must have an unlocked role ability.</p>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {eligibleShadowTargets.map((player) => (
                    ...
                    >
                      <span className="text-sm leading-none flex-shrink-0">{getPlayerIcon(player)}</span>
                      <span className="font-medium text-xs text-gray-100 flex-shrink-0">{player.name}</span>
                      <span className="text-xs text-purple-400 font-semibold">{player.role?.type}</span>
                      <span className="font-mono font-bold text-gray-400 text-xs ml-auto">📈 {player.projectMilestones}</span>
                    </Button>
                  ))}
                </div>
              </>
              ...
              <>
                <div className="mb-2">
                  <h4 className="text-xs font-semibold text-gray-100 m-0">
                    Step 2: Select Target for {gameState.players.find(p => p.id === selectedTarget)?.role?.type} Ability
                  </h4>
                  <p className="text-xs text-gray-400 mt-1">
                    You will copy {gameState.players.find(p => p.id === selectedTarget)?.name}'s {gameState.players.find(p => p.id === selectedTarget)?.role?.type} ability and use it on this target.
                  </p>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {alivePlayers.map((player) => (
                    <Button
                      ...
                      onClick={() => handleTargetSelect(player.id)}
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 ${
                        shadowTarget === player.id ? 'border-amber-500 bg-amber-500/10' : ''
                      }`}
                    >
                      <span className="text-sm leading-none flex-shrink-0">{getPlayerIcon(player)}</span>
                      <span className="font-medium text-xs text-gray-100 flex-shrink-0">{player.name}</span>
